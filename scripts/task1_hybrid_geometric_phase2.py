...
def _resolve_real_grasp_center_world(
    *,
            ...
            return link_midpoint_world, {
                **link_midpoint_log,
                ...
                "resolution_priority": [
                    ...
                    "legacy_fallbacks_if_unresolved",
                ],
                "fallback_used": False,
    ...
    center_world, log = _resolve_fingertip_midpoint_reference_position(
        stage=stage,
        dc=dc,
        articulation=articulation,
        robot_root_path=robot_root_path,
        arm_side=arm_side,
        requested_name_token="real_grasp_center_fingertip_end_midpoint",
    )
    if center_world is not None:
        ...
        return center_world, {
            **log,
            "grasp_center_definition": "midpoint_between_active_fingertip_end_references",
            "resolution_priority": [
                "actual_fingertip_frames_if_available",
                "calibrated_finger_link_tip_proxy_from_distal_bbox_face",
                "legacy_finger_link_midpoint",
                "explicit_fallback_proxy",
            ],
            "fallback_used": bool(log.get("fallback_used", False)),
            "close_critical_reference": True,
            **comparison_log,
        }

    legacy_center_world, legacy_log = _resolve_finger_midpoint_reference_position(
        stage=stage,
        dc=dc,
        articulation=articulation,
        robot_root_path=robot_root_path,
        arm_side=arm_side,
        requested_name_token="legacy_real_grasp_center_finger_link_midpoint",
    )
    if legacy_center_world is not None:
        fallback_comparison = _compare_with_diagnostic_link_midpoint(
            primary_world=np.array(legacy_center_world, dtype=float),
            primary_log=legacy_log,
        )
        return legacy_center_world, {
            **legacy_log,
            "fingertip_end_resolution_log": log,
            "source": "legacy_finger_link_pair_midpoint_fallback",
            "grasp_center_definition": "legacy midpoint between finger link references after fingertip-end resolution failed",
            "resolution_priority": [
                "actual_fingertip_frames_if_available",
                "calibrated_finger_link_tip_proxy_from_distal_bbox_face",
                "legacy_finger_link_midpoint",
            ...
            "fallback_used": True,
            "fallback_source": "legacy_finger_link_midpoint",
            "close_critical_reference": True,
            **fallback_comparison,
        }
        ...
        if np.isfinite(fallback).all():
            ...
            return fallback, {
                **log,
                ...
                "grasp_center_definition": "explicit fallback proxy after fingertip-end and legacy link-midpoint resolution failed",
                "resolution_priority": [
                    "actual_fingertip_frames_if_available",
                    "calibrated_finger_link_tip_proxy_from_distal_bbox_face",
                    "legacy_finger_link_midpoint",
...
def final_descent_local_ik(
    *,
    ...
    def target_pose_fn() -> np.ndarray:
        ...
        measured_world = np.array(real_center_world, dtype=float) if real_center_world is not None else current_point_b_world
        ...
        commanded_pose, _ = _pose_for_point_b_world(commanded_world, coord_transform, commanded_rpy, contact_control_offset)
        ...
        if call_count == 1 or call_count % int(args.trace_interval) == 0:
            ...
            sample = {
                    ...
                    "measured_world_semantics": "fingertip_end_midpoint_if_resolved_else_point_B_proxy",
    ...
    result = _execute_dualarmik_servo_phase(
        ...
        extra_details={
            ...
            "target_lock_frame": {
                ...
                "point_B_proxy_semantics": "legacy proxy kept for diagnostics; final descent commands the close-critical contact reference when resolved",
            ...
            "control_rule": "measure fingertip-end midpoint when resolved, clamp XY, clamp yaw, descend monotonically in Z, solve incrementally through DualArmIK",
        },
    )
...
def evaluate_close_gate(
    *,
    ...
    close_commit_zone_source = pre_close_gate.get("close_critical_metric")

    hard_blockers = {
        "close_commit_zone_pass": close_commit_zone_pass,
        "table_clearance_pass": bool(table_clearance_margin >= float(args.phase2_table_clearance_min)),
        "width_compatibility_pass": width_compatibility_pass,
        "orientation_error_pass": bool(orientation_error <= float(args.phase2_close_orientation_tolerance)),
    }
    soft_warnings = {
    ...
    return {
        ...
        "orientation_error_tolerance_rad": float(args.phase2_close_orientation_tolerance),
...
def main() -> int:
    ...
    try:
        ...
        if vertical_tip_rule_close_allowed:
            ...
            final_close_decision = {
                "condition_met": bool(close_allowed_by_primary or close_allowed_by_fallback),
                "allowed_by": "primary_phase2_close_gate"
                if close_allowed_by_primary
                else ("vertical_support_or_stall_fallback" if close_allowed_by_fallback else None),

#!/usr/bin/env python3
"""Task 1 Phase 1 hybrid geometric skeleton for Walker S2.

This script is copied from scripts/task1_dualarmik_phase_baseline.py and keeps
the existing official SceneBuilder, Walker S2 loading, DualArmIK backend, and
phase-machine execution. Phase 1 adds only a minimal table-frame, scene_state
object_info, finite candidate generation, deterministic candidate scoring, and
structured logging for future hybrid planner hardening.
"""
...
SCRIPT_NAME = "task1_hybrid_geometric_phase1.py"
LOG_STEM = "task1_hybrid_geometric_phase1"
SOURCE_BASELINE_SCRIPT = "scripts/task1_dualarmik_phase_baseline.py"
        ...
        if np.isfinite(fallback).all():
            return fallback, {
                **log,
                ...
                "grasp_center_definition": "phase1_explicit_fallback_proxy_after_finger_midpoint_resolution_failed",
                "resolution_priority": [
                    "effective_finger_references_if_available",
                    "finger1_link_finger2_link_midpoint",
...
def _execute_dualarmik_servo_phase(
    ...
    *,
    ...
    success = bool(final_pos_error <= float(spec.pos_tolerance) and final_rot_error <= float(spec.rot_tolerance))
    if not success:
...
def main() -> int:
    ...
    payload: dict[str, Any] = {
        "run_metadata": {
            ...
            "plan_phase": "Phase 1 - Minimal hybrid skeleton",
            "seed": int(args.seed),
            "gui_enabled": bool(args.no_headless or args.gui),
        },
        "motion_policy": {
            "architecture": "phase_based_official_dualarmik_6d_servo",
            "phase1_hybrid_skeleton_active": True,
            "phase1_hybrid_scope": "table_frame_scene_state_object_info_finite_candidates_deterministic_selection_only",
            "phase1_deferred": [
                "full_geometric_hardening",
                "Thinker_advisor",
                "YOLO_provider",
                "closed_loop_final_descent_hardening",
                "retry_taxonomy",
            ],
            "target_mode": args.target_mode,
    ...
    try:
        ...
        selected_hybrid_candidate, hybrid_candidate_selection = select_best_candidate(hybrid_candidates)
        ...
        payload["hybrid_phase1"] = {
            ...
            "table_unit_m": TABLE_UNIT_M,
            "perception_source": "scene_state",
            "candidate_generation_scope": "finite_phase1_candidates_only",
            ...
            "deferred_to_later_phases": [
                "rich_geometric_filtering",
                "local_closed_loop_descent_controller",
                "retry_with_next_candidate",
                "Thinker_reranking",
                "YOLO_provider",
            ],
        }
        ...
        if selected_hybrid_candidate is None:
            _fail("hybrid_phase1_no_valid_candidate", "No valid Phase 1 hybrid candidate passed coarse workspace/width sanity")
        print(
        ...
        close_ok = _command_gripper_phase(
            ...
            extra_details={"pre_close_gate": pre_close_gate},
        )

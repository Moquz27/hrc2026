...
Task 1 scene, reuses the baseline RobotArticulation camera interfaces, and
writes synchronized RGB-D arrays plus structured labels/metadata. It does not
run manipulation, replace the planner, or connect a Thinker policy.
"""
...
def _save_camera_arrays(
    *,
    ...
    for camera_name in CAMERA_NAMES:
        ...
        depth_summary = _array_summary(depth)
...
def _gfig_quat_to_xyzw(quat: Any) -> list[float]:
...
def _resolve_robot_frame(stage: Any, robot_prim_path: str) -> dict[str, Any]:
    ...
    quat_xyzw = _gfig_quat_to_xyzw(transform.ExtractRotationQuat())
...
def _class_for_part(stage: Any, prim_path: str, index: int, num_parts_per_class: int) -> dict[str, Any]:
    ...
    by_ref = _category_from_reference(refs)
    if by_ref != "unknown":
        return {"class": by_ref, "source": "usd_reference_path", "reference_paths": refs}
    by_order = "part_a" if index < num_parts_per_class else "part_b"
    return {"class": by_order, "source": "scene_builder_creation_order_fallback", "reference_paths": refs}
...
def _estimate_projection_visibility(
    *,
    ...
    return {
        ...
        "visibility_semantics": "best_effort_center_projection; labels include all spawned Task 1 parts by default",
...
def _labels_for_sample(
    *,
    ...
    for index, prim_path in enumerate(part_paths):
        ...
        class_payload = _class_for_part(stage, prim_path, index, num_parts_per_class)
    ...
    return {
        ...
        "label_policy": {
            ...
            "visibility_note": "Per-object visibility is best-effort projection metadata; no segmentation or occlusion filtering is applied in Phase 1.",
        },
    }
...
def _build_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--selected-arm", choices=("left", "right", "none"), default="none")
    parser.add_argument("--selected-preset", default=None)
...
def main() -> int:
    ...
    counter = {"step": 0}
    try:
        _log(log_path, "collector_start", run_id=run_id, samples=args.samples, gui=bool(args.gui or args.no_headless))
        ...
        run_metadata = {
            ...
            "implemented_scope": [
                ...
                "official RobotArticulation camera RGB-D interfaces",
                "JSONL manifest",
                "per-sample RGB/depth numpy arrays",
        ...
        for sample_index in range(int(args.samples)):
            ...
            runtime_metadata = {
                ...
                "selected_arm": None if args.selected_arm == "none" else args.selected_arm,
            ...
            sync_debug = {
                ...
                "sync_note": "RGB and depth are requested from each official camera wrapper without advancing sim_app.update between cameras.",
            }
            ...
            manifest_entry = {
                ...
                "sample_index": int(sample_index),
                "simulation_step": capture_step,
                "timestamp_utc": runtime_metadata["timestamp_utc"],
                "paths": {
